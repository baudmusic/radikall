...
现在可以把 PWA / 主屏 Web App 直接部署到 Cloudflare Workers，不再依赖需要持续付费的常驻服务器。

- 公网地址：[https://radikall-web.baudmusic.workers.dev](https://radikall-web.baudmusic.workers.dev)
- 公网入口：稳定的 `workers.dev` 地址
- 优先目标：iPhone Safari + 主屏 Web App
- API 与静态资源：由同一个 Worker 提供
- 播放链路：免费生产方案只保留 HLS，不再包含 `ffmpeg` / `audio.mp3` fallback
...
- Worker 配置：[wrangler.jsonc](E:/YLY/RADIKO/radiko-app/wrangler.jsonc)
...
- 完整部署文档：[cloudflare-workers-free-deploy.md](E:/YLY/RADIKO/radiko-app/docs/cloudflare-workers-free-deploy.md)

给用户的分发方式：

- 直接把 [https://radikall-web.baudmusic.workers.dev](https://radikall-web.baudmusic.workers.dev) 发给用户
- iPhone / iPad 用户用 Safari 打开后，按页面顶部提示“添加到主屏幕”
- GitHub Release 保留作更新日志和备用入口，不再作为 Web 版的主安装入口
