...
이제 PWA / 홈 화면 Web App을 Cloudflare Workers에 직접 배포할 수 있어서, 항상 켜져 있는 유료 서버에 의존하지 않아도 됩니다.

- 공개 URL: [https://radikall-web.baudmusic.workers.dev](https://radikall-web.baudmusic.workers.dev)
- 공개 진입점: 안정적인 `workers.dev` 주소
- 우선 대상: iPhone Safari + 홈 화면 Web App
- API 및 정적 자원: 동일한 Worker가 함께 제공
- 재생 경로: 무료 운영 경로는 HLS만 유지하며 `ffmpeg` / `audio.mp3` fallback은 포함하지 않습니다
...
- Worker 설정: [wrangler.jsonc](E:/YLY/RADIKO/radiko-app/wrangler.jsonc)
- Worker 진입점: [index.ts](E:/YLY/RADIKO/radiko-app/webApp/worker/src/index.ts)
- 정적 데이터 생성 스크립트: [generate-worker-data.mjs](E:/YLY/RADIKO/radiko-app/webApp/scripts/generate-worker-data.mjs)
- 전체 배포 문서: [cloudflare-workers-free-deploy.md](E:/YLY/RADIKO/radiko-app/docs/cloudflare-workers-free-deploy.md)

사용자 배포 방식:

- 그대로 [https://radikall-web.baudmusic.workers.dev](https://radikall-web.baudmusic.workers.dev) 주소를 전달합니다
- iPhone / iPad 사용자는 Safari로 열고, 화면 상단 안내에 따라 홈 화면에 추가합니다
- GitHub Release는 변경 로그와 보조 진입점으로만 남기고, Web 버전의 주요 설치 경로로 쓰지 않습니다
