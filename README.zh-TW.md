...
- Windows 平台生成**兩種安裝包格式**：`EXE` 和 `MSI`。
- Windows 打包需要**包含 `jpackage` 的完整 JDK**，而非部分 IDE 內建的精簡版 JetBrains Runtime。
- `assembleRelease` 目前生成 `androidApp-release-unsigned.apk`，發布給使用者前需要用你自己的 keystore 進行簽署。
