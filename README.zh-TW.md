...
現在可以把 PWA / 主屏 Web App 直接部署到 Cloudflare Workers，不再依賴需要持續付費的常駐伺服器。

- 公網地址：[https://radikall-web.baudmusic.workers.dev](https://radikall-web.baudmusic.workers.dev)
- 公網入口：穩定的 `workers.dev` 位址
- 優先目標：iPhone Safari + 主屏 Web App
- API 與靜態資源：由同一個 Worker 提供
- 播放鏈路：免費生產方案只保留 HLS，不再包含 `ffmpeg` / `audio.mp3` fallback
...
- 靜態目錄產生腳本：[generate-worker-data.mjs](E:/YLY/RADIKO/radiko-app/webApp/scripts/generate-worker-data.mjs)
- 完整部署文件：[cloudflare-workers-free-deploy.md](E:/YLY/RADIKO/radiko-app/docs/cloudflare-workers-free-deploy.md)

給使用者的分發方式：

- 直接把 [https://radikall-web.baudmusic.workers.dev](https://radikall-web.baudmusic.workers.dev) 發給使用者
- iPhone / iPad 使用者請用 Safari 開啟，並依頁面提示加入主畫面
- GitHub Release 保留作更新日誌與備用入口，不再作為 Web 版的主要安裝入口
